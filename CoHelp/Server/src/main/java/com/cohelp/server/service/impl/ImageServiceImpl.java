...
public class ImageServiceImpl extends ServiceImpl<ImageMapper, Image>
    ...
    @Override
    public Result setImageState(Map<Image, Integer> imageIntegerMap) {
        if(ObjectUtils.anyNull(imageIntegerMap)){
            return ResultUtil.fail(ERROR_PARAMS,"参数为空");
        }
        Set<Image> images = imageIntegerMap.keySet();
        if(images.isEmpty()){
            return ResultUtil.fail(ERROR_PARAMS,"参数不合法");
        }
        ...
        for(Image image : images ){
            ...
            Integer state = imageIntegerMap.get(image);
            //创建查询条件
