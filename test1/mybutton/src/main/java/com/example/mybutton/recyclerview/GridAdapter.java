...
public  class GridAdapter extends RecyclerView.Adapter<GridAdapter.LinearViewHodler> {
    ...
    @Override
    public GridAdapter.LinearViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        //传入一个view 在什么地方显示出对于的内容
        //不是传入activity_hor_re_view
        return new LinearViewHodler(LayoutInflater.from(mContext).inflate(R.layout.activity_gird_rv_item,parent,false));
    }

    @Override
    public void onBindViewHolder (GridAdapter.LinearViewHodler holder, @SuppressLint("RecyclerView") int position) {
    ...
    class LinearViewHodler extends RecyclerView.ViewHolder{
        ...
        public LinearViewHodler( View itemView) {
            super(itemView);
            textView = itemView.findViewById(R.id.tv_rv_grid);
        }
    }
